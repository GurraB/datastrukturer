...
public class ArrayHeap<E> {
	...
	public static <E> void sort(E[] elements) {
		ArrayHeap<E> heap = new ArrayHeap<E>(elements, new ReverseComparable<E>());
		heap.heapify();
	...
	public ArrayHeap(E[] elements, Comparator<E> comp) {
		this.list = elements;
		size = list.length;
		this.comp = comp;
		heapify();
...
//			heap.insert(i);
		ArrayHeap<Integer> heap = new ArrayHeap<Integer>(values);
		System.out.println();
		while(heap.size()>0) {
